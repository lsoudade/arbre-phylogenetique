...
abstract class Repository
{
     ...
     */
    public function findAll(array $fieldsToFilter = array(), array $fieldsToRetrieve = array(), array $fieldsToOrder = array(), $maxRows = null, $offset = 0)
    {
        // Execute query
        $qb = $this->db->createQueryBuilder()
            ->select(empty($fieldsToRetrieve) ? '*' : implode(',', $fieldsToRetrieve))
    ...
    {
        $query = sprintf('SELECT COUNT(id) as nb FROM %s', $this->tableName);
        $statement = $this->db->prepare($query);
        $statement->execute();
        $row = $statement->fetch();
